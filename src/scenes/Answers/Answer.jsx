...
const Answer = ({singleQuestion}) => {
  return (
    <>
        <BoxContainer>
            <h5 style={{fontWeight : "bold"}} ><span>{singleQuestion.id}</span> -  {singleQuestion.question}</h5>
            <BoxAnswer>
                {Object.values(singleQuestion.answers).map((answer, key ) => (
                    <Item>
                        {
                            (answer===singleQuestion.correctAnswer) ? 
